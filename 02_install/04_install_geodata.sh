install_geodata() {
    [ "$install_v2fly_geodata" = true ] && curl -L -o "$GEO_DIR/geodata_v2ray.dat" "https://github.com/v2fly/domain-list-community/releases/latest/download/dlc.dat" > /dev/null 2>&1
    ...
    # Добавляем обработку замены файлов
    [ "$update_v2fly_geodata" = true ] && [ -f "$GEO_DIR/geodata_v2ray.dat" ] && curl -L -o "$GEO_DIR/geodata_v2ray.dat" "https://github.com/v2fly/domain-list-community/releases/latest/download/dlc.dat" > /dev/null 2>&1
