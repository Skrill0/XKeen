choose_geoip() {
    ...
    [ "$GEO_EXISTS_GEOIP_V2RAY" != "installed" ] && has_missing_geoip_bases=true
    [ "$GEO_EXISTS_GEOIP_ANTIFILTER" = "installed" ] || [ "$GEO_EXISTS_GEOIP_V2RAY" = "installed" ] && has_updatable_geoip_bases=true

    while true; do
        # Вывод доступных действий
        echo 
        echo 
        echo "Выберите номер или номера действий для GeoIP:"
        echo 
        echo "0. Пропустить"
        ...
        [ "$GEO_EXISTS_GEOIP_V2RAY" != "installed" ] && v2fly_choice="Установить" || v2fly_choice="Обновить"
        ...
        for choice in $geoip_choices; do
            if [ "$choice" -eq 0 ]; then
                ...
                echo -e "${YELLOW}Все GeoIP уже установлены${RESET}."
                ...
                echo -e "${RED}Нет установленных GeoIP для удаления${RESET}. Пожалуйста, выберите другой пункт."
                invalid_choice=true
                break
            fi
        done
        ...
        if [ "$invalid_choice" = false ]; then
            ...
            for choice in $geoip_choices; do
                if [ "$choice" -eq 1 ]; then
                    ...
                    [ "$GEO_EXISTS_GEOIP_V2RAY" != "installed" ] && install_v2fly_geoip=true || update_v2fly_geoip=true
            ...
            # Экспорт переменных
            export chose_geoip_antifilter_select
            export chose_geoip_v2fly_select
            export chose_delete_geoip_antifilter_select
            export chose_delete_geoip_v2fly_select
